  ...
  try {
    ...
    if (result.passwordMode === 2 && !credentials.mailboxPassword) {
      throw new TwoFactorRequiredError(
        "Mailbox password required for this Proton account.",
      );
    }
    ...
    const basePassword =
      credentials.mailboxPassword?.trim() || credentials.password;
    ...
    try {
      secretStore.set(
        ...
        JSON.stringify(saltedPassphrases),
      );
    } catch (error) {
...
async function getKeySalts(
  ...
  try {
    ...
    if (safeError.message.toLowerCase().includes("scope")) {
      logger.warn(
        "Key salts unavailable due to scope",
        redactMeta({ correlationId, endpoint: "/core/v4/keys/salts" }),
      );
