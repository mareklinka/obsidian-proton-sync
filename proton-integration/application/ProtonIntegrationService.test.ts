...
function buildLogger(entries: LogEntry[]): ProtonLogger {
  return {
    ...
    warn: (message, meta, error) => entries.push({ level: 'warn', message, meta, error }),
    error: (message, meta, error) => entries.push({ level: 'error', message, meta, error })
  };
}
...
describe('createProtonIntegration', () => {
  ...
  it('getApiClient is null when disconnected and after error', async () => {
    ...
    await expect(integration.signIn({ email: 'a@b.com', password: 'pwd' })).rejects.toThrow();
  ...
  it('DI storage failure produces sanitized error', async () => {
    ...
    await expect(integration.signIn({ email: 'alice@example.com', password: 'x' })).rejects.toThrow(
      'Operation failed due to a secure authentication error.'
    );

    expect(integration.getStatus().lastError).toBe('Operation failed due to a secure authentication error.');
  });
});
