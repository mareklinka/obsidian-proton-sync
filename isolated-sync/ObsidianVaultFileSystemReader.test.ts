...
function makeReader(
  ...
  const customGuards = useDefaultGuards
    ? {}
    : {
      isFile: (entry: unknown): entry is never => Boolean(entry && typeof entry === "object" && (entry as FakeEntry).kind === "file"),
      isFolder: (entry: unknown): entry is never => Boolean(entry && typeof entry === "object" && (entry as FakeEntry).kind === "folder"),
    };
