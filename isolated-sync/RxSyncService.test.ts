...
describe("RxSyncService (isolated)", () => {
  ...
  it("validates oldPath for move/rename events", () => {
    ...
    expect(() =>
      service.enqueueChange({ type: "file-moved", entityType: "file", path: "new.md" }),
    ).toThrowError(/oldPath/i);
  });
  ...
  it("dispose is terminal", () => {
    ...
    expect(() =>
      service.enqueueChange({ type: "file-created", entityType: "file", path: "x.md" }),
